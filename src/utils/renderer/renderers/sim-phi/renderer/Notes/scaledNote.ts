export class ScaledNote {
    ...
    constructor(img: HTMLImageElement | ImageBitmap | HTMLCanvasElement, scale: number, compacted?: boolean) {
        this.img = img;
        this.scale = scale;
        ...
        this.body = (ctx: CanvasRenderingContext2D, offset: number, length: number): void => ctx.drawImage(img, dx, offset, dw, length);
        this.tail = (ctx: CanvasRenderingContext2D, offset: number): void => ctx.drawImage(img, dx, offset - dh, dw, dh);

        if (compacted) {
            this.head = (ctx: CanvasRenderingContext2D): void => ctx.drawImage(img, dx, dy, dw, dh);
            this.tail = (ctx: CanvasRenderingContext2D, offset: number): void => ctx.drawImage(img, dx, offset - dh - dy, dw, dh);
        }
    }
}