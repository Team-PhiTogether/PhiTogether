...
export async function readFile(
  ...
  if (name === "Settings.txt" || name === "info.txt") {
    const data = await stringify(i.buffer);
    const chartInfo = joinPathInfo(chartConverter.info(data), path);
    return { type: "info", data: chartInfo, buffer: i.buffer };
  }
  return new Promise(() => {
    ...
    .catch(async () => {
      const webp2canvas = Object.getOwnPropertyDescriptor(self, 'webp').value.default;
    ...
    .catch(async () => {
      ...
      try {
        ...
        try {
          if (parsedJSON.META) {
            const rpeData = chartConverter.parseRPE(data, i.name, path); //qwq
        ...
        return new Promise(() => {
          ...
          const pecData = chartConverter.parse(data, i.name);
