...
export default async function saveAdjustedChart(app, fucktemp2) {
    ...
    if (!loadedChartMeta || loadedChartMeta.isFromURL)
        downloadRPE(app);
    else {
        if (typeof app.chart.offset !== 'number' || !hook.chartsMD5.get(hook.selectchart.value)) shared.game.msgHandler.sendMessage(shared.game.ptmain.$t("simphi.adjustOffset.errorWhenSavingOffset"), "error");
        ...
        shared.game.msgHandler.sendMessage(shared.game.ptmain.$t("simphi.adjustOffset.offsetSaved"), "success");
    }
}
function downloadRPE(app) {
    ...
    } else return shared.game.msgHandler.sendMessage(shared.game.ptmain.$t("simphi.adjustOffset.onlyRPESupport"), "error");
    downloadText(
        dlfilename,
        text4dl,
        "application/json",
    );
    shared.game.msgHandler.sendMessage(shared.game.ptmain.$t("simphi.adjustOffset.adjustedChartSaved"), "success");
}
