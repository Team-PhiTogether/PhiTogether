...
export async function loadLineData() {
    ...
    for (const i of simphiPlayer.chartData.chartLineData) {
        if (simphiPlayer.selectchart.value === i.Chart) {
            ...
            /** @type {ImageBitmap} */
            const image =
                simphiPlayer.chartData.bgs.get(i.Image) || simphiPlayer.res["NoImageBlack"];
            simphiPlayer.app.lines[i.LineId].imageW = image.width;
            simphiPlayer.app.lines[i.LineId].imageH = image.height;
            if (!simphiPlayer.lineImages.has(image))
                simphiPlayer.lineImages.set(image, new LineImage(image));
            ...
            simphiPlayer.app.lines[i.LineId].isCustomImage = simphiPlayer.chartData.bgs.get(i.Image)
                ? true
                : false;
            ...
            if (isFinite((i.Horz = parseFloat(i.Horz))))
                simphiPlayer.app.lines[i.LineId].imageA = i.Horz; //Legacy
            ...
            if (isFinite((i.Scale = parseFloat(i.Scale))))
                simphiPlayer.app.lines[i.LineId].imageS = i.Scale;
            if (isFinite((i.Aspect = parseFloat(i.Aspect))))
                simphiPlayer.app.lines[i.LineId].imageA = i.Aspect;
