    ...
    for (const i2 of oldEvents) {
        ...
        if (i1.endTime > i2.endTime) {
            ...
            newEvents.push({
                ...
                endTime: i2.endTime,
                start:
                    (i2.start * (i2.endTime - i1.endTime) + i2.end * (i1.endTime - i2.startTime)) /
...
export function reverse(chart, duration) {
    ...
    for (const i of chart.judgeLineList) {
        ...
        for (const j of i.speedEvents)
            judgeLine.speedEvents.push(new SpeedEvent(j.value, tb - j.endTime, tb - j.startTime));
        for (const j of i.notesAbove) {
            if (j.type === 3)
                judgeLine.notesAbove.push(
                    new Note(j.type, tb - j.time, j.positionX, 1, j.speed * j.holdTime)
                );
            else
                judgeLine.notesAbove.push(
                    new Note(j.type, tb - j.time, j.positionX, j.holdTime, j.speed)
                );
            judgeLine.numOfNotesAbove++;
        }
        for (const j of i.notesBelow) {
            if (j.type === 3)
                judgeLine.notesBelow.push(
                    new Note(j.type, tb - j.time, j.positionX, 1, j.speed * j.holdTime)
                );
            else
                judgeLine.notesBelow.push(
        ...
        for (const j of i.judgeLineDisappearEvents)
            judgeLine.judgeLineDisappearEvents.push(
                new JudgeLineEvent(
                    j.end,
                    j.start,
                    j.end2,
        ...
        for (const j of i.judgeLineMoveEvents)
            judgeLine.judgeLineMoveEvents.push(
                new JudgeLineEvent(
                    j.end,
                    j.start,
                    j.end2,
        ...
        for (const j of i.judgeLineRotateEvents)
            judgeLine.judgeLineRotateEvents.push(
                new JudgeLineEvent(
                    j.end,
                    j.start,
                    j.end2,
