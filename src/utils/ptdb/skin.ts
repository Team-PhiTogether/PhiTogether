  ...
  return new Promise(async (res, rej) => {
    if (
      ...
      (await shared.game.msgHandler.confirm(
        shared.game.i18n.t("skin.haveCommon")
      ))
    )
      return res();
...
export function getAllSkins(
  dbName: string = ObjectStores.Skin
): Promise<CachedSkin[]> {
  return new Promise((res, rej) => {
    openDB()
      .then((db) => {
...
export function deleteSkin(id: string | null): Promise<boolean> | null {
  ...
  return new Promise((res, rej) => {
      openDB()
          .then(db => {
              const objStore = db.transaction([ObjectStores.Skin], 'readwrite').objectStore(ObjectStores.Skin);
