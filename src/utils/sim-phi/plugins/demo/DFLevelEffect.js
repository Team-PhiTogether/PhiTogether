      ...
      animationCurve: [
        {
          time: 0,
          value: 0,
          inSlope: -0.0021248339,
          outSlope: -0.0021248339,
          inWeight: 0,
          outWeight: 1
        },
        {
...
import { getValue } from "./ease.js";
const getAttr = time => {
  const attributes = {};
  for (const [key, value] of Object.entries(obj)) {
    attributes[key] = getValue(value, time);
  }
  return attributes;
};
export function loadMod() {
  return time => {
    ...
    if (time > 125.2 && time < 150) {
      ...
      const countDownSec = String(Math.floor(countDown)).padStart(2, "0");
      const countDownCentiSec = String(Math.floor(countDown * 100) % 100).padStart(2, "0");
      let countDownText = "";
      ...
      hook.tmps.customForeDraw = ctxos => {
        ctxos.globalAlpha = 1;
        ctxos.fillStyle = "#fff";
        ctxos.fillRect(wlen - lineScale * 1.625, lineScale * 0.25, lineScale * 3.25, lineScale * 1.75);
        ctxos.fillStyle = "#000";
        ctxos.textAlign = "center";
        ctxos.font = `${lineScale * 1.0}px Saira`;
        ctxos.fillText(countDownText, wlen, lineScale * 1.15);
        ctxos.font = `${lineScale * 0.4}px Saira`;
        ctxos.fillText("COUNTDOWN", wlen, lineScale * 1.73);
      };
    }
  };
}