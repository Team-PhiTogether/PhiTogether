...
function parse(pec, filename) {
    ...
    const result = { formatVersion: 3, offset: 0, numOfNotes: 0, judgeLineList: [], chartRPE: JSON.parse(pec) };
    const warnings = [];
    ...
    data.judgeLineList.forEach((i, index) => {
        i.LineId = index;
        if (typeof i.father === "number" && i.father !== -1) i.father = data.judgeLineList.length - 1 - i.father;
    });
    //bpm变速
    ...
    for (const i of data.judgeLineList) {
        ...
        if (i.notes) {
            for (const note of i.notes) {
                ...
                lineRPE.pushNote(type, time, positionX, holdTime, speed, note.above === 1, note.isFake !== 0, note.size, note.alpha, note.visibleTime);
            }
        }
        ...
        if (i.extended) {
            if (i.extended.colorEvents) for (const j of i.extended.colorEvents) {
                if (!j) continue;
                const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
                const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
                const start = j.start;
            ...
            if (i.extended.textEvents) for (const j of i.extended.textEvents) {
                if (!j) continue;
                const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
                const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
                const start = j.start;
            ...
            if (i.extended.scaleXEvents) for (const j of i.extended.scaleXEvents) {
                if (!j) continue;
                const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
                const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
                const start = j.start;
            ...
            if (i.extended.scaleYEvents) for (const j of i.extended.scaleYEvents) {
                if (!j) continue;
                const startTime = bpmList.calc(j.startTime[0] + j.startTime[1] / j.startTime[2]);
                const endTime = bpmList.calc(j.endTime[0] + j.endTime[1] / j.endTime[2]);
                const start = j.start;
