                    ...
                    shared.game.ptmain
                        ...
                        .then(async () => {
                            if (!this.taptapToBind) this.$router.back();
                                ...
                                if(await msgHandler.confirm(this.$t("login.askTapTapBinding",{username:this.taptapToBind.name}))) {
                                    msgHandler.sendMessage(this.$t("login.bindingTapTap"));
                                    phizoneApi.bindTapTap(e.access_token, this.taptapToBind.unionId)
                                        .then(e=>{
                                            msgHandler.sendMessage(this.$t("login.bindTapTapSuccess"), "success", true);
                                            this.$router.back();
                                        })
                                        .catch(()=>{
                                            msgHandler.sendMessage(this.$t("login.bindTapTapFailure"), "error", true);
                                        })
                                } else {
                                    this.$router.back();
                                }
...
<template>
    <div id="loginPage" class="routerRealPage">
        ...
        <div class="loginPageRow">
            {{ $t("login.email") }}：<input class="input textInput" style="width:calc(100%/2);" v-model="username" autocomplete="username" />
        </div>
        <div class="loginPageRow">
            {{ $t("login.passwd") }}：<input class="input textInput" v-model="password" style="width:calc(100%/2);" type="password"
                autocomplete="current-password" @keyup.enter="doLogin" />
        </div>
        <div class="loginPageRow">
            <input type="button" style="width:auto;font-size:1.5em;" :value="$t('login.signin')" @click="doLogin()" />
            <input type="button" style="width:auto;font-size:1.5em;" :value="$t('login.signup')" @click="jumpReg()" />
        </div>
        <h1 class="loginPageRow" style="font-size:2em;" v-if="taptapAvailable">{{ $t("phizone.login.withTapTap") }}</h1>
        <div class="loginPageRow">
            <img src="/src/core/loginWithTapTap.png" style="width:10em;" v-if="taptapAvailable" @click="loginWithTapTap()" alt="Login With TapTap">
        </div>
    </div>
</template>
