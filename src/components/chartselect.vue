...
export default {
    ...
    computed: {
        ...
        canPrev() {
            if (["pz", "favorite"].includes(this.selectChoice)) return this.chartList.hasPrevious;
    ...
    methods: {
        ...
        loadPrevPage() {
            if (["pz", "favorite"].includes(this.selectChoice)) this.loadPagePZv2(this.page - 1);
            else if (["local", "custom"].includes(this.selectChoice)) {
        ...
        async playPreview(para) {
            ...
            ptdb.chart.song
                ...
                .then(async e => {
                    ...
                    if (this.selectedSongData.id !== para.id || this.$route.path !== "/chartSelect")
                        return;
                    audio.play(bfs, {
                        loop: true,
            ...
            if (this.selectChoice === "pz") {
                ...
                this.beforePagination = this.beforeSearch.filter(x => {
                    return (
                        ...
                        x.illustrator.toLowerCase().includes(this.search.illustrator.toLowerCase())
                    );
                });
                this.page = 1;
                this.updatePagination();
            } else if (["custom", "favorite"].includes(this.selectChoice)) {
                if (!this.chartList.resultsbak) this.chartList.resultsbak = this.chartList.results;
        ...
        async loadFavouriteSongs() {
            ...
            chartList.results = await ptdb.chart
                ...
                .then(results => results.filter(result => this.favouriteSongs.includes(result.id)));
            this.chartList = chartList;
            document.querySelector("#app").scrollTop = 0;
            shared.game.loadHandler.r();
        },
        ...
        selectChart(chart) {
            ...
            if (!chart.userScore) {
                ...
                const scoreData = (chart.isFromPhiZone
                    ...
                    : shared.game.ptmain.gameConfig.ptBestRecords)[chart.id] || [0, 0, false, true];
                if (!scoreData[3]) {
        ...
        async deleteChart() {
            if (!(await shared.game.msgHandler.confirm(this.$t("cacheManage.confirmBeforeDelete"))))
                return;
