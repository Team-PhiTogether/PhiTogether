(function (window) {
  ...
  function loop() {
    ...
    for (let i = 0; i < numOfBubbles; i++) {
      ...
      let radius =
        curBubble.baseRadius *
        map(curBubble.y, 0, window.innerHeight + 75, 0, 1);
      ctx.globalAlpha =
        curBubble.baseAlpha *
        map(curBubble.y, 0, window.innerHeight * 0.5 + 75, 0, 1);
      ...
      */
      ...
      if (i % 2 === 1) {
        curBubble.x +=
          Math.sin((Date.now() * SHAKE_RATE) / 4 + curBubble.sinOffset) * 2;
      } else {
        curBubble.x -=
          Math.sin((Date.now() * SHAKE_RATE) / 4 + curBubble.sinOffset) * 2;
      }
